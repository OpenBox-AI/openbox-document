...
The governed-command integration provides an enforcement path for explicitly registered commands: `OpenBoxPlugin(..., sandbox=SandboxConfig(...))` owns one-attempt scheduling, and exact `CONSTRAIN` selects sandbox execution. It does not sandbox every constrained Temporal action. See [Governed Sandbox Commands](/developer-guide/temporal-python/governed-sandbox-commands) for the native Worker composition.
