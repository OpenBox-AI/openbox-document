...
4. Run `obs status` and confirm the selected provider is `srt` (the default).
5. Load `~/.config/openbox-sandbox/agent.env` into the Worker environment and confirm its service, policy, and mTLS paths exist.

Keep Worker registration and interception inside that `OpenBoxPlugin` initializer. The launcher never falls back to another provider if `srt` provisioning fails. See [Governed Sandbox Commands](/developer-guide/temporal-python/governed-sandbox-commands) for the supported composition.
