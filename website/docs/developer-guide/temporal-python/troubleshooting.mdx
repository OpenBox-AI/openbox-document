...
description: "Agent registration failing? Plugin timeout? Fix the 5 most common OpenBox setup issues in under 2 minutes with this troubleshooting guide."
...
If a registered command reports `GovernedCommandConfigurationRequired`:

1. Confirm the native Worker has `plugins=[OpenBoxPlugin(...)]`.
2. Confirm that same plugin initializer receives `sandbox=SandboxConfig(registry=command_registry)`.
3. Confirm the requested profile exists in that registry.
4. Run `obs status` and confirm the selected provider is `native` (the default).
5. Load `~/.config/openbox-sandbox/agent.env` into the Worker environment and confirm its service, policy, and mTLS paths exist.

Keep Worker registration and interception inside that `OpenBoxPlugin` initializer. The launcher never falls back to another provider if `native` provisioning fails. See [Governed Sandbox Commands](/developer-guide/temporal-python/governed-sandbox-commands) for the supported composition.

---

## Governance Blocks or Stops Your Agent

When a behavioral rule or policy blocks an operation, the plugin raises a non-retryable `GovernanceBlock` or `GovernanceHalt` error. This is expected — it means governance is working.
...
See **[Error Handling](/developer-guide/temporal-python/error-handling)** for how to handle governance and approval errors in your code.
