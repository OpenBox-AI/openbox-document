...
Keep Worker registration and interception inside that `OpenBoxPlugin` initializer. The launcher never falls back to another provider if `native` provisioning fails. See [Governed Sandbox Commands](/developer-guide/temporal-python/concept) for the supported composition.
