...
5. Step 4, Enforcement. Set the verdict to `CONSTRAIN` and type `post-batch`
   into **Profile ID**. That box accepts any text, and the console cannot see
   your registry, so it cannot check the name. Type the exact `command_id`
