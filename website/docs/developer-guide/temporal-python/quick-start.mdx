...
Download the launcher into a directory you own, then provision. The `v0.1.1-dev` tag selects the development release line, whose default policy permits `/usr/bin/curl` to reach `example.com:443`. [Provisioning](./provisioning) explains the release lines and every flag.

```bash
curl -fL -O https://github.com/OpenBox-AI/openbox-sandbox/releases/download/v0.1.1-dev/obs-darwin-arm64
curl -fL -O https://github.com/OpenBox-AI/openbox-sandbox/releases/download/v0.1.1-dev/SHA256SUMS
...
# Required for a behavioral rule with an HTTP trigger. Core classifies a span
# as `http_get` only when it carries `http.method` and its name contains GET,
# which is what this instrumentation emits; without it no trigger event is
# recorded and the rule never fires. The upper bound matches the SDK's own
# `[http]` extra — unpinned, resolution conflicts with its opentelemetry-api pin.
uv add 'opentelemetry-instrumentation-httpx>=0.59b0,<0.61b0'
```
...
class PaymentBatchWorkflow:
    ...
    async def run(self, batch: dict) -> dict:
        return await workflow.execute_activity(
            ...
            # A governed activity that cannot reach its sandbox fails
            # retryably, and Temporal retries without limit by default,
            # so an unreachable sandbox would spin instead of failing.
        ...
        plugins=[
            OpenBoxPlugin(
                ...
                sandbox=SandboxConfig(
                    ...
                    # Gives the governed dispatcher its own Core client.
                    # Without it the dispatcher builds the
                    # `openbox.sandbox_execution` evidence span and then
                    # discards it, so the console shows the run with no
                    # sandbox evidence to expand.
...
Two files, complete. The workflow calls two activities so the run shows both
sides: `post_payment_batch` makes the HTTP call and is therefore the activity
the rule constrains, and `compute_payment_total` performs no I/O and always
runs on the host.
...
@activity.defn
async def post_payment_batch(batch: dict) -> dict:
    """The behavioral rule constrains THIS one.

    It makes the HTTP call, so it is the activity that emits the `http_get`
    span. Core matches a rule's trigger against the current event's spans, so
    an `http_get` trigger can only fire on the activity that made the call.
    Under CONSTRAIN this body never runs.
    """
    async with httpx.AsyncClient() as client:
        response = await client.get("https://example.com")
...
async def compute_payment_total(batch: dict) -> dict:
    """Always runs on the host. This is NOT the constrained activity.

    It performs no I/O, so it emits no spans and can never match an `http_get`
    trigger. `computed_by: host` appears on every run, including a successful
    CONSTRAIN, and is not a failure signal.
    """
...
The workflow returns both activities. `posting` is the constrained one: its
body never ran, so `activity_result` is `None` and the registered command ran
in the sandbox instead. `total` ran on the host untouched.

```text
{'posting': {'activity_result': None,
             'sandbox_execution': {
                 'cleanup_status': 'deleted',
                 'disposition': 'executed_in_sandbox', 'exit_code': 0,
                 'profile_id': 'post-batch', 'status': 'completed',
                 'stderr_bytes': 0, 'stdout_bytes': 66,
                 'timeout_status': 'not_observed',
                 'typed_result': {'schema_name': 'sandbox-http',
                                  'values': [{'name': 'http_status', 'value': 200},
                                             {'name': 'remote_ip', 'value': '127.0.0.1'},
                                             {'name': 'local_ip', 'value': '127.0.0.1'}]}}},
 'total': {'batch_id': 'B-2026-001', 'computed_by': 'host'}}
```

`activity_result: None` beside a populated `sandbox_execution` is the CONSTRAIN
signature. `computed_by: host` under `total` is expected on every run and does
not indicate a missed constraint.
...
You provisioned the base release line, which denies every destination. Provision from the `v0.1.1-dev` tag.
