...
1. Open **Agent > Authorize > Behaviors > Create Rule**.
2. Name the rule and set a priority.
3. Set the trigger to `http_get`. The first activity fetches a page, which
   emits that event.
4. Leave the required prior states empty. The form does not require them.
5. Set the verdict to `CONSTRAIN` and the profile to `post-batch`.
6. Write an on-reject message, then create the rule.

Disable any policy rule that matches `post_payment_batch`. That rule would stop
the first activity before it makes the call that fires this trigger.
...
Start from the same two files, then make three additions. The behavioral rule
needs a first activity that performs an action and a second activity to
constrain.

Add the second activity to `__main__.py`:
...
Register it beside the first one:

```python title="__main__.py"
activities=[post_payment_batch, compute_payment_total],
```

Everything else stays the same, including the registry. Both rule types run the
same profile, `post-batch`.
