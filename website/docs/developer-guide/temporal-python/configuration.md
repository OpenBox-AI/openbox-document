...
The sandbox runtime defaults to the `native` provider. Provision it with `obs provision --provider native --yes` (or omit `--provider` because `native` is the default), then load `~/.config/openbox-sandbox/agent.env`. Additional providers are documented separately.
