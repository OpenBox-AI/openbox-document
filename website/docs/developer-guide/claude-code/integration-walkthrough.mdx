...
If the session's changes are committed, the commit carries an `OpenBox-Session` trailer linking it back to this session; see [Agent Lineage → Shift-Left](/core-concepts/agent-lineage#shift-left-governance).
