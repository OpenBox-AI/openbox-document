...
`npm run openbox:admin:setup` mutates the configured OpenBox agent. Use it only when you intend to refresh the demo's OpenBox-side guardrails, behavior rules, and policy setup. If you already configured your own controls in Authorize, skip that setup script.
