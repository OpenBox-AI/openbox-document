...
| `OPENBOX_API_URL` | server-side OpenBox platform/backend URL for approval decisions submitted from the CopilotKit UI |
| `OPENBOX_BACKEND_API_KEY` | server-side org/backend key for posting CopilotKit UI approval decisions back to OpenBox |

`OPENBOX_BACKEND_API_KEY` is not sent to the browser and is not required for normal runtime governance.
