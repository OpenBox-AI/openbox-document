...
const createSupportTicket = tool(
  async ({ customerId, summary, priority }) => {
...
`createSupportTicket` is a normal LangChain tool. OpenBox is added by `createOpenBoxCopilotKitAdapter()` and the middleware it creates. A file named `openbox_governance.ts` in the reference app is only a local helper that wraps the adapter setup above.
