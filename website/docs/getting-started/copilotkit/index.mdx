...
:::info Backend model
OpenBox is layered above CopilotKit. In the current LangGraph path, the SDK also installs middleware in the backend so CopilotKit and the graph share one governed session. Future CopilotKit backend bridges can follow the same layering.
:::
...
CopilotKit does not create OpenBox agents or rules for you. Start from the same OpenBox setup used by the other integrations:
