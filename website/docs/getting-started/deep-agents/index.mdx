...
OpenBox integrates with [DeepAgents](https://github.com/langchain-ai/deepagents) by wrapping your agent with a middleware layer — your subagents, tools, and graph logic stay exactly as they are while every action is governed, scored, and auditable.
...
Full developer reference: middleware hooks, per-subagent targeting, HITL conflict detection, and all configuration options.
...
OpenBox SDK initialized successfully
  ...
  - Known subagents: researcher, writer
```
...
The **Event Log Timeline** shows the full execution trace — model calls, tool calls, subagent dispatches, and governance decisions. Click **Watch Replay** to open [Session Replay](/trust-lifecycle/session-replay) for step-by-step playback.
...
- **Tracked subagent dispatches** — tagged each action with the dispatching subagent (`researcher`, `writer`) for per-subagent policy targeting
