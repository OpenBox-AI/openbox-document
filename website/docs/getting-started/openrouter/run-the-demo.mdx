...
| **Verify → Run Receipt** | Build the receipt, download it, and verify it offline |
...
Download a receipt from **Verify → Run Receipt**, then:

```bash
openbox-verify --keys openbox-keys.json receipt.json
```

Turn your network off first if you want to watch it work. See [Run Receipt](/trust-lifecycle/run-receipt) for what each check establishes.
