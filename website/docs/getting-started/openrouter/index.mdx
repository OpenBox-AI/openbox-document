...
OpenBox integrates with the [OpenRouter Agent SDK](https://openrouter.ai/docs/agent-sdk) by wrapping `callModel` and your tools. Your agent stays as it is; OpenBox adds policy evaluation before each call runs, approvals, telemetry, and [Proof of Routing](/core-concepts/proof-of-routing) — the record of which upstream provider actually served each prompt.
...
| Which provider served each prompt, in which region, at what cost | [Routing Integrity](/dashboard/routing-integrity) |
| A provider allowlist the gateway fails closed on | [Routing Policies](/developer-guide/openrouter/routing-policies) |
| Signed evidence you can hand to a customer | [Run Receipt](/trust-lifecycle/run-receipt) |
...
4. **[Proof of Routing](/core-concepts/proof-of-routing)** - Why the record exists and what it can prove
