---
title: Run the Demo
description: "Run the OpenBox demo locally: See runtime governance, policy enforcement, and trust scoring in action with example Temporal workflows."
llms_description: Hands-on demo walkthrough
sidebar_position: 2
tags:
  - getting-started
  - temporal
...
- **[Node.js 22+](https://nodejs.org/)** — Required for the demo frontend
...
| 2 | `make run-worker` | Runs the Temporal worker that executes your agent's workflow and activity code. The OpenBox plugin is initialized here, intercepting every event for governance |
...
When you ran the demo, the OpenBox plugin:
...
- **Recorded one of five governance decisions for every event** — `ALLOW`, `CONSTRAIN`, `REQUIRE_APPROVAL`, `BLOCK`, or `HALT` — giving you a complete audit trail
...
- **[Configure Trust Controls](/trust-lifecycle/authorize)** — Set up guardrails, policies, and behavioral rules for LLM interactions
