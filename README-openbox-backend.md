...
Build the image:
```bash
docker build -t openbox-backend .
```

Run the container (map port 3000):
```bash
docker run --name openbox-backend -p 3000:3000 --env-file .env -e NODE_ENV=production openbox-backend
```

Run the container in the `openbox` network:
```bash
docker run --name openbox-backend --network openbox -p 3000:3000 --env-file .env -e NODE_ENV=production openbox-backend
```
